...
class Yedit(object):
    ...
    def add_entry(data, key, item=None, sep="."):
        ...
        for arr_ind, dict_key in key_indexes[:-1]:
            if dict_key:
                ...
                raise YeditException(
                    "Unexpected item type found while going through key path: {}".format(
                        key
                    )
                )
        ...
        # expected list entry
            ...
            and int(key_indexes[-1][0]) <= len(data) - 1
        ):  # noqa: E501
    ...
    def write(self):
        ...
        try:
            Yedit._write(
                self.filename, yaml.dump(self.yaml_dict, Dumper=yaml.RoundTripDumper)
            )
        except AttributeError:
            Yedit._write(
    ...
    def parse_value(inc_value, vtype=""):
        ...
        if isinstance(inc_value, str) and "bool" in vtype:
            if inc_value not in true_bools and inc_value not in false_bools:
                raise YeditException(
                    "Not a boolean type. str=[{}] vtype=[{}]".format(inc_value, vtype)
                )
