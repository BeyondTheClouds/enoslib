...
and connect to them over SSH as root. For this purpose you must have a SSH key file
available in ``~/.ssh`` (no matter the encryption). Using the standard Grid'5000
...
::
    ...
    signatures)
    ─────────────────────────────────────────────────────────────────────────────────────────────────────────
    fio-3.25 running on parasilo-6.rennes.grid5000.fr: average /dev/md0 read performance = 495.78 IOPS
    fio-3.25 running on parasilo-7.rennes.grid5000.fr: average /dev/md0 read performance = 352.92 IOPS
...
For this much more complex example, we use the `servan cluster
<https://www.grid5000.fr/w/Grenoble:Hardware#servan>`_  on which every node has
3 usable physical network interfaces.  In addition, this example includes many advanced
features:
...
:download:`tuto_grid5000_kavlan_complex.py <grid5000/tuto_grid5000_kavlan_complex.py>`
...
.. code-block:: python
    ...
    [x] provider.hosts[0]
    <G5kHost(roles=['control'], fqdn=servan-1.grenoble.grid5000.fr, ssh_address=servan-1-kavlan-4.grenoble.grid5000.fr, primary_network=<G5kVlanNetwork(roles=['my_network'], site=grenoble, vlan_id=4)>, secondary_networks=[<G5kVlanNetwork(roles=['my_second_network'], site=grenoble, vlan_id=5)>])>

    # Another example (what are the hosts in the same network as me)
    [x] provider.hosts[0].primary_network.hosts
    [<G5kHost(roles=['control'], fqdn=servan-1.grenoble.grid5000.fr, ssh_address=servan-1-kavlan-4.grenoble.grid5000.fr, primary_network=<G5kVlanNetwork(roles=['my_network'], site=grenoble, vlan_id=4)>, secondary_networks=[<G5kVlanNetwork(roles=['my_second_network'], site=grenoble, vlan_id=5)>])>,
     <G5kHost(roles=['control', 'compute'], fqdn=servan-2.grenoble.grid5000.fr, ssh_address=servan-2-kavlan-4.grenoble.grid5000.fr, primary_network=<G5kVlanNetwork(roles=['my_network'], site=grenoble, vlan_id=4)>, secondary_networks=[<G5kVlanNetwork(roles=['my_second_network'], site=grenoble, vlan_id=5)>])>]
