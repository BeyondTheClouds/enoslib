...
.. hint::
    ...
    - Most of the example make use of a ``try/except/finally`` block that eases
      the functionnal testing phase. If you want to explore them (any exception
      will delete the corresponding job), it's probably better to get rid of
      this block (at least in the first steps).

    - |enoslib| has also a dedicated tutorial at https://msimonin.gitlabpages.inria.fr/enoslib-tutorials/
